...
`skills-check [--templates DIR]` requires every template in the pinned bundle to exist at `.agents/skills/<name>/SKILL.md` with the same logical text, so a consumer copy cannot drift from the shared procedure. Line endings alone are not drift: a Windows checkout may materialise the same content as CRLF. Additional consumer-only skills are allowed; templates are located beside the executable unless `--templates` names a directory.

`generate-check` compares trees with slash-form relative paths and ignores CRLF versus LF, so Windows runners do not fail when `gofmt` or another generator rewrites LF while the checkout still holds CRLF. Currency pack hashes and `--check` / `--app-check` comparisons use the same LF-normalized digest, and generators always write LF.
...
`changes` returns `code`, `backend`, `ui`, `visual`, `policy`, `mutation`, `fuzz`, `native`, `docs_only` booleans and a `files` array. `fuzz` and `mutation` follow the packages and isolated-copy includes the policy's own commands name, so renaming a file inside a gated package keeps the gate while an unrelated file whose name merely contains a keyword does not add one; a configured gate whose paths cannot be read is treated as affected, and an unconfigured gate is not triggered. Unknown flags are rejected instead of ignored. `ui` also triggers native suites for policy changes; `visual` identifies production UI changes requiring before/after evidence. Local calls without a base compare uncommitted/untracked work with HEAD; they do not review already committed feature-branch work. Use `--base <target-ref>` for that branch, and set `LEDGER_BASE` for the coverage step inside `check`. CI provides the PR base SHA on pull requests and the previous tip (`github.event.before`) on pushes. An all-zero before (first push of a ref) or an orphan `HEAD^` resolves to an empty base: `changes` still treats all tracked source as affected so suites run, while `coverage-check` reports incremental coverage as not applicable and enforces only the overall floor — it must not reinterpret the whole tree as "changed code" subject to the 90% gate. A real merge-base (PR base or prior tip) still requires 90% on intersecting executable lines. Governance and shared-foundation changes broaden verification. Missing eligible coverage evidence is an error, not zero changed code.
